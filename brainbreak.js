...
function openRandomPickerModal(){
  ...
  const card = document.createElement("div");
  card.className = "actModalCard activityPickerCard";
  ...
  const title = document.createElement("div");
  title.className = "actModalTitle";
  title.textContent = "Pick a Brain Break";
  ...
  const body = document.createElement("div");
  body.className = "actModalBody activityPickerBody";

  const intro = document.createElement("div");
  intro.className = "pickerIntro";
  intro.textContent = `${pool.length} enabled activities. Roll one, then use it or roll again.`;

  const result = document.createElement("div");
  result.className = "randomResultCard";

  const resultIcon = document.createElement("div");
  resultIcon.className = "randomResultIcon";
  resultIcon.textContent = "🧠";

  const resultName = document.createElement("div");
  resultName.className = "randomResultName";
  resultName.textContent = "Ready to roll";

  const resultMeta = document.createElement("div");
  resultMeta.className = "randomResultMeta";
  resultMeta.textContent = "Click Roll";

  const resultSteps = document.createElement("div");
  resultSteps.className = "randomResultSteps";
  resultSteps.textContent = "Students can watch the choice appear.";

  result.appendChild(resultIcon);
  result.appendChild(resultName);
  result.appendChild(resultMeta);
  result.appendChild(resultSteps);

  const btnRow = document.createElement("div");
  btnRow.className = "pickerActions";
  ...
  manualBtn.textContent = "Choose manually";
  ...
  function showCandidate(act){
    if (!act) return;
    resultIcon.textContent = activityIcon(act);
    resultName.textContent = act.name;
    resultMeta.textContent = activityMeta(act);
    resultSteps.textContent = activityPreview(act);
  }
  ...
  function roll(){
    ...
    result.classList.remove("isPicked");

    let count = 0;
    const total = 14 + Math.floor(Math.random() * 8);

    function step(){
      const temp = pickFromPool(pool);
      showCandidate(temp);

      count++;

      if (count < total){
        const delay = 45 + count * 10;
        setTimeout(step, delay);
        return;
      }

      chosen = pickFromPool(pool);
      showCandidate(chosen);

      rolling = false;
      useBtn.disabled = false;
      rollBtn.disabled = false;

      result.classList.add("isPicked");
      setTimeout(()=> result.classList.remove("isPicked"), 280);
    }
  ...
  // Auto-roll once so the button feels alive immediately.
  roll();
}
...
function openChooserModal(){
  ...
  search.type = "text";
  search.placeholder = "Search by name, type, tag, or instruction…";
  search.className = "activitySearch";
  search.autocomplete = "off";

  const filterRow = document.createElement("div");
  filterRow.className = "activityFilterRow";
  ...
  filters.forEach(([key, label])=>{
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "activityFilterBtn";
  ...
  countLine.className = "pickerCountLine";

  const list = document.createElement("div");
  list.className = "activityChoiceList";
  ...
  function render(){
    ...
    const filtered = pool.filter(act=>{
      const group = activityGroup(act);
      const hay = `${act.name} ${act.tag || ""} ${act.type || ""} ${(act.steps || []).join(" ")}`.toLowerCase();
    ...
    countLine.textContent = `${filtered.length} shown • ${pool.length} enabled`;

    if (!filtered.length){
      const empty = document.createElement("div");
      empty.className = "emptyPickerState";
      empty.textContent = "No matches. Try All, or clear the search box.";
      list.appendChild(empty);
      return;
    }

    filtered.forEach(act=>{
      list.appendChild(makeActivityChoiceCard(act, (picked)=>{
        setCurrent(picked);
        closeAllOverlays();
      }));
    });
